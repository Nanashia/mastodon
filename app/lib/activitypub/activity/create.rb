...
class ActivityPub::Activity::Create < ActivityPub::Activity
  ...
  def process_poll
    ...
    expires_at = begin
      if @object['closed'].is_a?(String)
        @object['closed']
      elsif @object['closed'].is_a?(TrueClass)
        Time.now.utc
      else
        @object['endTime']
      end
  ...
  def poll_vote?
    return false if replied_to_status.nil? || replied_to_status.poll.nil? || !replied_to_status.local? || !replied_to_status.poll.options.include?(@object['content'])
    replied_to_status.poll.votes.create!(account: @account, choice: replied_to_status.poll.options.index(@object['content']))
  end
